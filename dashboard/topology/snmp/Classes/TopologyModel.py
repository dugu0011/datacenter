from dashboard.topology.snmp.helpers import log
import json
import re
import pprint
import copy
...
class NetworkTopology:
    ...
    def __init__(self):
        ...
        try:
            with open(base + 'interface_stats.json') as json_file:
    ...
    def addDevice(self, s_device_name):
        if not self.isDeviceDefined(s_device_name):
            ...
            self.visualization_dict['nodes'].append({"id": s_device_name, "group": group_number, "image": image_name , "protocol" : "snmp"})
    ...
    def dumpToJSON(self):
        ...
        for device in self.interface_stats:
            for interface in self.interface_stats[device]['interfaces']:
                filename = base + 'stats/' + device + '_' + str(interface['index']) + ".json"
                #log(filename)
        ...
        with open(base + 'neighborships.json', 'w') as outfile:
        ...
        with open(base + 'no_neighbor_interfaces.json', 'w') as outfile:
